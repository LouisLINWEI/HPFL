...
class ML_Dataset(): 
    ...
    def get_datasets(self, dataset_name):
        if dataset_name == 'Mnist':
            train_dataset = datasets.MNIST(root='/data/share_data/htj_fl/mnist/', train=True, transform=transforms.ToTensor(), download=True)
            test_dataset = datasets.MNIST(root='/data/share_data/htj_fl/mnist/', train=False, transform=transforms.ToTensor())
            class_num = 10
        if dataset_name == 'EMnist':
            train_dataset = datasets.MNIST(root='/data/share_data/htj_fl/emnist/', train=True, transform=transforms.ToTensor(), download=True)
            test_dataset = datasets.MNIST(root='/data/share_data/htj_fl/emnist/', train=False, transform=transforms.ToTensor())
            class_num = 62
        if dataset_name == 'Cifar10':
            train_dataset = datasets.CIFAR10(root='/data/share_data/htj_fl/cifar10/', train=True, transform=transforms.ToTensor(), download=False)
            test_dataset = datasets.CIFAR10(root='/data/share_data/htj_fl/cifar10/', train=False, transform=transforms.ToTensor())
            class_num = 10
        if dataset_name == 'KWS':
