    ...
    def get_datasets(self, dataset_name):
        ...
        if dataset_name == 'EMnist':
            if not os.path.exists('/data/emnist/'):
                os.mkdir('/data/emnist/')
            train_dataset = datasets.MNIST(root='/data/emnist/', train=True, transform=transforms.ToTensor(), download=True)
            test_dataset = datasets.MNIST(root='/data/emnist/', train=False, transform=transforms.ToTensor())
            class_num = 62
        if dataset_name == 'Cifar10':
            if not os.path.exists('/data/cifar10/'):
                os.mkdir('/data/cifar10/')
            train_dataset = datasets.CIFAR10(root='/data/cifar10/', train=True, transform=transforms.ToTensor(), download=False)
