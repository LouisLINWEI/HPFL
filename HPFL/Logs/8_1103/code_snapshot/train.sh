...
for i in $workers
do
	command="source /root/miniconda3/bin/activate && conda activate flamby && export CUDA_VISIBLE_DEVICES=0,1,2,3 && python3 $home/$job --master_address=tcp://${master}:$port_no --rank=$num --world_size=$world_size --remarks=$remarks --trial_no=$trial_no"
	# echo $command
